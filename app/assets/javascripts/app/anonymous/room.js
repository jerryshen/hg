...
var page = {
  ...
  //目前图片写死代替
  // initVideo:function(){
  //  $("#video")[0].src = ".//image"+util.getIdTag()+".jpg";
  ...
  buildMsgUI:function(item){
    ...
    return ['<div class="item" data-id="'+item.idClient+'">',
          '<p class="nick '+type+' j-nick '+roleClass+'" data-role="'+type+'" data-account="'+item.from+'">'+((type==="owner"||type==="manager")?'<b class="role role-'+type+'">管理员</b>':"")+item.fromNick+'</p>',
  ...
  buildChatMemberUI:function(data){
    ...
    return  ['<div class="item" data-id="'+data.account+'">',
            (type==="owner"||type==="manager")?'<b class="role role-'+type+'">管理员</b>':'',
            '<div class="avatar">',
